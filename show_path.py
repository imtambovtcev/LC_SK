...
def path(file, show=False, states=True):
    ...
    np.savez(str(save_dir.joinpath('info.npz')), dist=dist, dist_norm=dist / dist.max(), energy=energy,
             energy_from_ref=energy_from_ref, epu=epu,
    ...
    print(
        f'max at {argrelextrema(energy_from_ref, np.greater)},\twith {energy_from_ref[argrelextrema(energy_from_ref, np.greater)]}')
    print(
        f'min at {argrelextrema(energy_from_ref, np.less)},\twith {energy_from_ref[argrelextrema(energy_from_ref, np.less)]}')
