...
def plot_state(system, s, directory, file, show_extras=False):
    print(f'{system.size = }')
    print(f'{len(system.size) = }')
    if len(system.size) == 3:
        print('3D system')
        ...
        fig.savefig(_directory.joinpath(file + '_xz.pdf'), bbox_inches='tight')
        plt.close('all')
        fig, _, _ = magnes.graphics.plot_field3D(system, state, slice2D='yz', sliceN=int(system.size[0] / 2))
        fig.savefig(_directory.joinpath(file + '_yz.pdf'), bbox_inches='tight')
        plt.close('all')
        fig, _, _ = magnes.graphics.plot_field3D(system, state, slice2D='xy', sliceN=int(system.size[2] / 2))
        fig.savefig(_directory.joinpath(file + '_xy.pdf'), bbox_inches='tight')
        plt.close('all')
        fig, _, _ = magnes.graphics.plot_field3D(system, state, slice2D='xy', sliceN=0)
        fig.savefig(_directory.joinpath(file + '_xy_0.pdf'), bbox_inches='tight')
        plt.close('all')
        if show_extras:
            plt.plot(s[int(system.size[0] / 2), int(system.size[1] / 2), :, 0, 2])
            plt.xlabel(r'$z$')
            plt.ylabel(r'$m_z$')
            plt.savefig(_directory.joinpath(file + '_z.pdf'), bbox_inches='tight')
            plt.close('all')
            plt.plot(s[int(system.size[0] / 2), int(system.size[1] / 2), :, 0, 0])
            plt.ylabel(r'$m_x$')
            plt.xlabel(r'$z$')
            plt.savefig(_directory.joinpath(file + '_x.pdf'), bbox_inches='tight')
            plt.close('all')

            # toplogical charge
            toplogical_charge = [utilities.toplogical_charge(system, s, i) for i in range(system.size[2])]
            plt.plot(toplogical_charge)
            plt.ylabel(r'$Charge$')
            plt.xlabel(r'$z$')
            plt.savefig(_directory.joinpath(file + '_tch.pdf'), bbox_inches='tight')
            plt.close('all')

            toplogical_charge_vertical = [utilities.toplogical_charge_vertical(system, s, i) for i in
                                          range(system.size[0])]
            plt.plot(toplogical_charge_vertical)
            plt.ylabel(r'$Charge$')
            plt.xlabel(r'$x$')
            plt.savefig(_directory.joinpath(file + '_tch_v.pdf'), bbox_inches='tight')
            plt.close('all')
    elif len(system.size) == 2:
        print('2D system')
        _directory = Path(directory)
        state = system.field3D()
        state.upload(s)
        fig, _, _ = magnes.graphics.plot_field(system, state)
        fig.savefig(_directory.joinpath(file + '.pdf'), bbox_inches='tight')
        plt.close('all')
    else:
        print('System size unknown')
        ...
        if 'STATE' in container:
            ...
            if container['PATH'].shape[0] == 1:
                ...
                for idx, s in enumerate(container['PATH']):
                    plot_state(system=system, s=s, directory=file.parent.joinpath(file.stem),
							   file=file.stem + '_{}'.format(idx), show_extras=show_extras)

    except:
        print('failed')


def show(_dir, show_extras=True):
    _directory = Path(_dir)
    if _directory.is_dir():
        print(f'{_directory = }')
        filelist = [file for file in _directory.iterdir() if file.suffix == '.npz']
        print(filelist)
        print(len(filelist))
        for file in filelist:
            plot_npz(file, show_extras=show_extras)
    else:
        file = Path(_directory)
        if file.suffix == '.npz':
            plot_npz(file, show_extras=show_extras)
